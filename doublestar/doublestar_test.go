...
var matchTests = []MatchTest{
	...
	{[]string{"[]a]"}, []string{"]"}, false, path.ErrBadPattern, true},
	{[]string{"[-]"}, []string{"-"}, false, path.ErrBadPattern, true},
	{[]string{"[x-]"}, []string{"x"}, false, path.ErrBadPattern, true},
	{[]string{"[x-]"}, []string{"-"}, false, path.ErrBadPattern, true},
	{[]string{"[x-]"}, []string{"z"}, false, path.ErrBadPattern, true},
	{[]string{"[-x]"}, []string{"x"}, false, path.ErrBadPattern, true},
	{[]string{"[-x]"}, []string{"-"}, false, path.ErrBadPattern, true},
	{[]string{"[-x]"}, []string{"a"}, false, path.ErrBadPattern, true},
	{[]string{"\\"}, []string{"a"}, false, path.ErrBadPattern, !onWindows},
	{[]string{"[a-b-c]"}, []string{"a"}, false, path.ErrBadPattern, true},
	{[]string{"["}, []string{"a"}, false, path.ErrBadPattern, true},
	{[]string{"[^"}, []string{"a"}, false, path.ErrBadPattern, true},
	{[]string{"[^bc"}, []string{"a"}, false, path.ErrBadPattern, true},
	{[]string{"a["}, []string{"a"}, false, nil, false},
	{[]string{"a["}, []string{"ab"}, false, path.ErrBadPattern, true},
	...
	{[]string{"ab{c,d}["}, []string{"abcd"}, false, path.ErrBadPattern, true},
...
func testMatchWith(t *testing.T, idx int, tt MatchTest) {
	...
	if isStandardPattern(pattern) {
		...
		if ok != stdOk || !compareErrors(err, stdErr) {
			t.Errorf("#%v. Match(%#q, %#q) != path.Match(...). Got %v, %v want %v, %v",
				idx, pattern, testPath, ok, err, stdOk, stdErr)
		}
	}
}
...
func testPathMatchWith(t *testing.T, idx int, tt MatchTest) {
	...
	if isStandardPattern(pattern) {
		...
		if ok != stdOk || !compareErrors(err, stdErr) {
			t.Errorf("#%v. PathMatch(%#q, %#q) != filepath.Match(...). Got %v, %v want %v, %v",
				idx, pattern, testPath, ok, err, stdOk, stdErr)
		}
	}
}
...
func testGlobWith(t *testing.T, idx int, tt MatchTest, basepath string) {
	...
	if isStandardPattern(pattern) {
		...
		if !compareSlices(matches, stdMatches) || !compareErrors(err, stdErr) {
			t.Errorf("#%v. Glob(%#q) != filepath.Glob(...). Got %#v, %v want %#v, %v",
				idx, pattern, matches, err, stdMatches, stdErr)
		}
	}
}
