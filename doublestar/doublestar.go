...
//
func Glob(pattern string) (matches []string, basedir string, err error) {
	...
	if isWindowsUNC || patternComponents[0] == volumeName {
		...
		dir := fmt.Sprintf("%s%s", volumeName, string(os.PathSeparator))

		return doGlob(dir, patternComponents[startComponentIndex:], matches)
	}
...
// Perform a glob
func doGlob(basedir string, components, matches []string) ([]string, string, error) { // nolint funlen
	m := matches
	...
	patIdx := findVariableComponent(components)
	...
	patAtInx := components[patIdx]
	if patAtInx == "**" {
		// if the current component is a doublestar, we'll try depth-first
		for _, file := range files {
			// if symlink, we may want to follow
			if (file.Mode() & os.ModeSymlink) != 0 {
				if file, err = os.Stat(filepath.Join(basedir, file.Name())); err != nil {
					return m, basedir, err
				}
			}

			if file.IsDir() {
				if lastComponent { // recurse into directories
					m = append(m, filepath.Join(basedir, file.Name()))
				}

				if m, _, e = doGlob(filepath.Join(basedir, file.Name()), components[patIdx:], m); e != nil {
					return m, basedir, e
				}
	...
	match := false
	patAfterIdx := components[patIdx+1:]

	for _, file := range files {
		if match, e = matchComponent(patAtInx, file.Name()); e != nil {
			return m, basedir, e
		}
		...
		if match {
			if lastComponent {
				m = append(m, filepath.Join(basedir, file.Name()))
			} else if m, _, e = doGlob(filepath.Join(basedir, file.Name()), patAfterIdx, m); e != nil {
				return m, basedir, e
			}
		}
	}
...
func findVariableComponent(components []string) int {
	for patIdx := 0; patIdx < len(components); patIdx++ {
...
func matchComponent(pattern, name string) (bool, error) { // nolint gocognit
